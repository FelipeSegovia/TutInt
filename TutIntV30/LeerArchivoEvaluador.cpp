...
LeerArchivoEvaluador::LeerArchivoEvaluador() {
	conectorEvaluador = ConectorEvaluador::obtenerConectorEvaluador();
}

LeerArchivoEvaluador::~LeerArchivoEvaluador() { }

void LeerArchivoEvaluador::lecturaEvaluador() {
	ifstream archivo("baseConocimientoEvaluador.txt");
	if (archivo.fail()) {
	...
	while (!archivo.eof()) { // Se lee cada regla (una por linea)
		...
		while (constante && entrar) {
			...
			//conectorEvaluador->agregarHecho(hechoCabeza);
			entrar = false;
		}
		...
		for (it = hechos.begin(); it != hechos.end() && acceder; it++) {
			...
			while (constante && entrar) {
				...
				//conectorEvaluador->agregarHecho(newHecho);
		...
		conectorEvaluador->agregarRegla(new Regla(hechoCabeza, vectHecho));
